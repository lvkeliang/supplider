...
- [x] **TR-02b 附件下载链接也需 fetch+Blob 化（TR-02 新发现）**：`SupplierDetail.tsx` 附件列表仍用 `<a href={apiUrl(url)} target="_blank">`，在 Tauri webview 内 Content-Disposition: attachment 的保存行为未经真机验证（可能不弹保存框/无反馈）；应复用 `api.download()` + toast。未纳入 TR-02（范围限定 4 个导出/备份/模板入口）。——2026-09-12
...
- [x] **TR-16 Emoji 替换为 SVG 图标**：用 Heroicons 或 Lucide 替换 ⚠ 🚫 ✓ ⚖ 🔔 ⏰ 📍 等状态 emoji，保证跨平台一致性——2026-09-10
- [x] **TR-17 暗色模式支持**：利用 Tailwind `dark:` 前缀添加暗色模式，跟随系统或手动切换（`darkMode:class` + index.html 无闪烁内联脚本 + `useDarkMode` hook 持久化 + Header 开关；页面/card/input/btn/chip 全量 `dark:` 变体）——2026-09-14
- [x] **TR-18 键盘快捷键**：添加 Ctrl+N（新建）、/或 Ctrl+K（搜索）、Esc（返回）等快捷键，设置中提供说明——2026-09-10
...
- [ ] **企查查/天眼查 API 对接**：入库时按企业名称+统一社会信用代码自动填充工商信息；API Key 加密存储；费用预算上限与预警。**已预留**：`risk_flags` 字段保留外部信号（被执行人/行政处罚）不被引擎覆盖；个人版无 API，故新增 `POST /suppliers/{id}/risk-flags` 人工标记/清除该字段 + 详情页「外部信号录入」按钮（淘汰-phase 风险预警，manual 路径已落地 2026-09-14）
...
- [x] **自然语言搜索**："杭州本地能做市政工程的二级资质以上供应商" → LLM 转结构化 Filter——共享 `internal/nlsearch.Parse`（prompt+JSON 解析+资质校验+评分钳制）三端复用：HTTP `POST /ai/nl-search`（TR-19-D）+ CLI `srm-cli ainl`（经 sidecar 端点）+ MCP `nl_search_suppliers` tool；无 Key → ErrAIDisabled 降级——2026-09-14
...
- [x] **操作审计日志**：录入/修改/归档/恢复/黑名单/移出黑名单/合并/批量导入/导出全记录，跨供应商轨迹 newest-first 持久化（settings 键 `audit.trail`，随库备份跨重启），`GET /api/v1/audit` + 设置页「操作审计日志」卡（动作/名称/时间）+ CLI `srm-cli audit`（管理员终端/脚本可查）；actor=local（个人版单用户，多用户字段预留）——2026-09-14
