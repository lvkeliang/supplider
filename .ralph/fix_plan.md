...
- [ ] **TR-01 全局 Toast/通知基础设施**：应用不存在 Toast 组件，所有写操作（创建/编辑/黑名单/归档）成功后无反馈。引入轻量 Toast（`ToastProvider` + Context + Portal，约 40 行零依赖），所有 `submit()` 成功后 `toast('✓ 操作成功')`。涉及 `App.tsx` 全局
- [ ] **TR-02 所有下载操作添加反馈**：4 个下载入口（Excel 模板/导出 Excel/JSON 备份/zip 备份）全部零反馈。用 `fetch()` + `Blob` 替代 `a.click()` 可追踪下载完成，下载前 `toast('⬇ 准备中…')`，完成后 `toast('✓ 下载完成')`。涉及 `SupplierList.tsx`/`ImportView.tsx`/`SettingsView.tsx`
...
- [ ] **TR-04 地域填写省市级联选择器**：当前 3 个纯文本 `<input>` 无级联无校验，可输入"火星"。改用 `chinese_regions` 数据包 + 三个级联 `<select>`；可选 `onPaste` 正则拆分整段地址。涉及 `SupplierForm.tsx`
- [ ] **TR-05 供应商类型选项扩展**：当前仅 6 项 `<datalist>`，改为 `<select>` 9 大分类（施工承包/设计咨询/物资供应/设备租赁/物流运输/检测认证/技术服务/劳务服务/其他）+ 选"其他"展开自由文本。涉及 `SupplierForm.tsx`
...
- [ ] **TR-19 备注**：`compact()` 函数将 0 视为"未设置"问题也需修复（评分为 0 的供应商无法通过 `min_rating=0` 筛选）；设置页本地偏好保存成功消息应 3-5 秒自动消失；合并重复按钮可改为"合并重复档案…"带省略号风格
