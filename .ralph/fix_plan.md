...
- [~] **TR-19-E 文档分析搜索 + 语义搜索**：文档上传+向量化存储；`Gateway.Embed()` 实现（bge-m3 模型）；文档分析推荐供应商。**阶段 5，5-7 天**——后端（Gateway.Embed + `vectorstore` 端口/memory 实现 + `POST /ai/index` + `/ai/semantic-search` + `/ai/doc-search`）+ 前端（「AI 文档搜索」视图：粘贴/上传需求 → 提取需求 → 推荐列表+匹配理由，入口按 `ai_doc_search`=CanEmbed 门控，feature flag 拆分 AIDocSearch 随 CanEmbed 而非 Enabled）已落地 2026-09-13，待持久化 SQLite 向量表 + PDF/Office/图片文档解析
