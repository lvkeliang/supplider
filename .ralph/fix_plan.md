...
- [~] **TR-19-E 文档分析搜索 + 语义搜索**：文档上传+向量化存储；`Gateway.Embed()` 实现（bge-m3 模型）；文档分析推荐供应商。**阶段 5，5-7 天**——后端（Gateway.Embed + `vectorstore` 端口/memory+SQLite 持久化实现 + `POST /ai/index` + `/ai/semantic-search` + `/ai/doc-search`）+ 前端（「AI 文档搜索」视图，入口按 `ai_doc_search`=CanEmbed 门控）+ `vectorfactory` 接线（personal=SQLite `vectors.db` 持久化、small_business/enterprise=memory 占位待 Qdrant/Milvus）已落地 2026-09-13，待 PDF/Office/图片文档解析
