...
审计全文搜索对 FTS5 特殊语法的健壮性（用户在搜索框输入 `" * ( ) OR NEAR : % _` 等可能让
MATCH 表达式语法报错→列表/搜索接口 500）。结论：现有防护正确——长词全部以
`ftsQuote` 双引号短语绑定（内嵌引号成对转义），短词走参数化 LIKE（escapeLike 转义
%/_/\），协同分支的 bigram/front/back/fragment 也都是 LIKE 占位符，无任何裸拼接用户输入；
内存适配器本就用子串匹配无解析器。22 个病态关键词（含 `"`、`**`、`AND NOT`、`杭州 OR`、
未闭合引号、`a:b`、反斜杠、制表符等）对真实 SQLite 库 List 均不报错。把探针固化为
`TestFTSPathologicalKeywordsNeverErrors`（永久回归，防 ftsQuote 日后被改坏），并断言
病态输入后正常长词搜索仍命中。通知存储顺带复核：INSERT OR IGNORE + dedup 折叠、按
dedup_key 回填、空 key 离散事件恒插入、超限裁剪，内存/SQLite 两适配器语义一致，无问题。
验证：default+personal 全量绿、三 tag 编译、gofmt/vet 净。本轮为纯加固，无行为变更。
...
导入模板别名一致以自动映射）。JSON bundle 是全量保真备份（含 change_log/附件/自定义
字段），XLSX 为人工交换格式。SetCellValue 以字符串写入（"=" 开头也不会被当公式，无
CSV/公式注入）；自定义字段按全库并集生成列、空行留空；空库导出仍为合法工作簿；资质按
既有文档约定只导出最高等级（多资质无法塞进单行，全量走 JSON）。发现 XLSX 相比导入器
**少了两列可无损往返的字段**：网址（website）与主营产品（products）——导出后再导入会
静默丢失。补齐：固定列在"详细地址"后加"网址"、"品类"后加"主营产品"（标签即导入器
label/别名，自动映射），产品名逗号连接（导入器按逗号/顿号拆分）。往返测试在样例中加
website 与两条产品线并断言无损。绩效评分列仍不导出（每供应商多条评价且导入模板无项目/
日期列，本质无法单行列全），与资质同属"XLSX 非全量、全量用 JSON"的既定边界。
验证：default+personal 全量绿、三 tag 编译、gofmt/vet 净。

### 2026-09-09：附件按引用计数删除——合并共享引用后不再悬空/孤儿

审计附件上传/下载/删除链路。上传顺序正确（先写字节（临时文件+rename、按实际字节而非
header size 限 50MB）后写记录，记录失败则清理孤儿文件）；object key 服务端生成且
localfs.resolve 防穿越/控制字符；下载固定 Content-Disposition: attachment（内联渲染被
浏览器阻止）；CORS 只允许 tauri:// 与 loopback 源（DNS rebinding 带自己的源被拒，
不会反射任意 Origin）。发现一个由**合并引用共享**引入、旧的"按 key 前缀删除"启发式无法
处理的双向缺陷：

- 合并把重复方的附件记录以原 URL（字节仍存在 `<dupID>/` key 下）复制到主体。旧删除逻辑
  仅当 key 前缀等于当前供应商 id 才删字节：
  1. **悬空引用（数据丢失方向）**：合并后恢复被归档的重复方→在重复方上删附件→key 前缀
     匹配→删字节，而主体仍引用该 URL（下载 404）。
  2. **孤儿文件（磁盘泄漏方向）**：在主体上删合并来的附件→key 前缀是重复方 id→永不删字节。
- 修复：新增 `Service.AttachmentReferenced(url)`（全库分页扫描，含归档/黑名单记录，任一
  文档仍引用即 true）；handler 删除记录后只在**最后一个引用消失**时删字节，去掉 key 前缀
  启发式。引用扫描出错时保守保留字节（孤儿可恢复，误删不可恢复）。单文档普通删除语义不变。
- 测试：服务层两例——合并→恢复→在重复方删除（主体仍引用，不删）→主体再删（可删）；
  单文档删除即无引用。HTTP 层一例（真实 localfs）：字节在第一次 DELETE 后仍 200、
  最后一次 DELETE 后 404。
- 验证：default+personal 全量绿、三 tag 编译、gofmt/vet 净。

### 2026-09-09：列表/分页层审计——未知资质等级不再静默关闭硬过滤；补本地优先分页属性测试

审计 SQLite 列表查询构造（keyset 游标、本地优先排序 CASE、结构化过滤、资质等级映射）。
分页主体经属性测试确认正确：30 行混合地域库 × 7 行小页 × 3 种排序（created_at/rating/
name）× 2 方向，均无重复/遗漏、本地供应商整体在前；本地优先 CASE 在 keyset 谓词中出现
两次、ORDER BY 中一次的参数绑定计数（3×3）正确；内存适配器同测一致；qual_rank 在每次
Put 时从 doc 重算（编辑资质后无陈旧列）。发现一处**静默失效的硬过滤**并修复：

- **`min_qual_level=肆级`（或 "1级"/typo）静默关闭资质过滤**：`domain.QualRank` 对未知
  等级返回 0，而 SQL 谓词是 `qual_rank >= ? IF rank>0`——显式要求资质硬过滤时，一个无法
  识别的等级被当成 0（不过滤），返回**全部**供应商（含无资质者）且无任何提示。UI 用固定
  下拉不会触发，但 CLI/MCP/API 手填会。修复：domain 加 `QualRankKnown`；HTTP
  `filterFromQuery` 对非空但无法识别的等级返回 **400**（list 与 export 两处，错误信息列出
  合法等级）；MCP search_suppliers 返回可纠正的工具文本错误。CLI 把等级作为 query 参数
  传递，服务端 400 自动透传。空值仍表示不过滤。
- **补本地优先分页属性测试**：此前 prio CASE + keyset + ORDER BY 这条最复杂 SQL 零覆盖，
  现以属性测试固化（无 dup/missing、本地全在前、三排序×两方向）。
- 测试：HTTP 3 例（list 未知 400、7 个合法等级 200、export 未知 400）、MCP 未知等级工具
  错误而合法等级正常、SQLite 分页属性 6 子用例。真机 HTTP 复验肆级/1级 400、二级 200。
- 验证：default+personal 全量绿、三 tag 编译、gofmt/vet 净。

### 2026-09-09：空壳检测规则修复——15 位旧工商注册号不再误判伪造，资本千分位/成立日期中文写法生效

审计非 AI 空壳特征检测引擎（risk 包，纯本地可解释规则，R1xx 身份/R2xx 画像年龄/R3xx
财务；任一 high 或两条 medium 置 shell_risk）。规则阈值与"只标记不阻断"哲学无问题，
发现一处严重误报与两处静默漏报：

- **15 位旧版工商注册号被判"疑似伪造"（高危误报）**：三证合一（2015）前注册的公司持
  15 位纯数字工商注册号而非 18 位统一社会信用代码，真实老企业大量存在；旧规则对任何
  非 18 位代码一律 R102 high → shell_risk=true。实测资料完整的 16 年老公司仅凭该字段
  被标空壳。修复：15 位纯数字识别为旧注册号，降为 **R104 low**（提示补全 18 位代码，
  不入风险、不置 shell）；其他错误长度/含字母的 15 位仍 R102 high。
- **注册资本千分位逗号被截断**：`capitalRe` 在 "1,000万" 上只匹配到 "1"（无单位后缀），
  按 1 元解析，误触 R301 低资本信号。解析前去除半角/全角逗号。
- **成立日期只认 ISO 导致年轻公司规则漏报**：与上轮到期日同类问题——Excel/迁移数据里
  "2012/3/10"、"2012.3.10"、"2012年3月10日" 无法解析时 R201 静默跳过（真实空壳信号
  漏检）。risk 包独立于 supplier（不能反向依赖），本地对齐四种布局 + RFC3339。
- 测试：新增 3 例——旧注册号 R104 且不置 shell、5 位垃圾/15 位含字母仍 R102；
  "1,000万"/"1，000.50万元" 不触 R301 而 99 万触发；四种日期写法下成立 60 天的公司
  均触 R201。既有 6 例规则测试不变。
- 验证：default+personal 全量绿、三 tag 编译、gofmt/vet 净。
- 遗留（可接受，不改）：同一类型+等级但证书号不同的资质会触 R204（换证/原件核验提示，
  medium 且文案要求人工核原件）；认缴/实缴不区分；这些都是"提请人工复核"而非判定。
...
（<3s 红线，既有测试实测）、空行跳过、逐行错误不中断批次、评分解析容错均无问题；
发现一个**非确定性数据 bug** 和一个静默丢列：

- **重复固定字段列时 `range map` 随机决定取哪列**：表头"名称"与"公司名称"都自动映射到
  company_name（别名重叠），`Build` 用 `for col, key := range mapping`（Go map 迭代
  随机序），两列值冲突时**每次导入结果可能不同**——探针 200 次构建出现 19/181 分裂。
  修复：列按索引升序处理；标量字段首个非空列稳定优先（首列即主键约定）；品类/产品等
  列表字段多列**并集去重**；custom 同表头多列用"；"拼接而非随机覆盖；评分列首个
  **可解析**值优先（无法解析的 0 值不阻挡后列）。
- **Inspect 自动建议对重复别名也制造冲突映射**：改为首个匹配列占固定字段、其后重复
  表头转 `custom`（以表头为 key 保留其值，符合文档式存储"不丢数据"）。
- **未知 field key 静默丢列**：客户端 typo（`compny_name`）时该列被静默忽略、导入照常
  成功。现 Build 对未知键返回错误（HTTP 400），合法集合 = 字段表 + custom + 空（忽略）。
- 测试：新增 3 例——双公司名列 50 次构建恒取左列且 Inspect 只把首列建议为字段、双品类
  列并集+双 custom 表头拼接（20 次稳定）、未知键报错；既有"坏分仍生成空评分记录"
  语义保持（非空评分列即建记录，不可解析值计 0，不影响行导入）。真机 HTTP 复验：
  typo 映射 400 且错误信息明确，正常映射 200。
- 验证：default+personal 全量绿（含 1000 行性能测试）、三 tag 编译、gofmt/vet 净。
- 方法论：这是继"哨兵错误映射""可选体解码错误"后第三类输入陷阱——**map 迭代序依赖**。
  今后任何"多来源写入同一字段"的处理必须先把 key 排序，禁止裸 `range map` 决定覆盖。

### 2026-09-09：资质到期日解析接受中文常见写法——斜杠/点分/年月日日期不再静默失去提醒

延续时间驱动逻辑审计，本轮走查**资质到期提醒（提前 90/30/7 天）**。窗口分桶（expired/
7d/30d/90d 边界：<0 / 0–7 / 8–30 / 31–90，到期当天算 0 天在 7d 桶，与可见性缓冲
"当天仍在缓冲"一致）、UTC 午夜对齐、按紧急度排序、归档排除、每窗口通知去重均无误。
发现一个**静默功能失效**：

- `parseExpiryDate` 只认零填充 `2006-01-02` 与 RFC3339；写入路径（Create/Update）
  对 expiry **不做任何校验**，扫描时无法解析就静默跳过。用户/CLI/迁移数据按中文习惯
  填 `2026/10/1`、`2026.10.1`、`2026年10月1日` 或不带零填充的 `2026-10-1`，该证书
  永远不会产生任何临期/过期提醒，且无任何反馈——核心维护功能形同失效。
- 修复：解析器改为按布局列表依次尝试 `2006-1-2` / `2006/1/2` / `2006.1.2` /
  `2006年1月2日`（Go 数字布局同时接受零填充与非零填充，已实测八种组合），保留
  RFC3339；真正无法解析的（"长期有效"等）仍静默跳过（长期证书不应报错）。原始字符串
  在报告中原样展示，只用于日期计算时归一。全仓仅此一个到期日解析点，无重复逻辑。
- 测试：旧例把 `2026/09/01` 当"坏日期"钉为跳过——更正为未来斜杠日期（2026/09/15，
  +7 天）并断言进入 7d 桶、原始字符串保留；新增 7 种写法（ISO/斜杠/点分/年月日
  ×零填充/非填充）在冻结时钟 +10 天全部 days_left=10；坏字符串/空串仍跳过。真机
  personal 二进制 HTTP 复验：创建 expiry=`2026/9/29` 的资质，/reminders/expiring
  返回 30d 提醒（修复前 count=0）。
- 走查确认无问题的点：50,000 条 MaxExportDocs 截断是安全阀（个人版实际不可达，与
  Export/风险队列同模式）；黑名单供应商仍属"在库"记录参与提醒（可见可搜，保守保留）；
  Excel 导入当前不映射到期日列（只有 qual_type/level），故无 Excel 日期序列号问题；
  通知按 证书号→类型+等级+到期日 身份去重，换证产生新事件。
- 后挂（非本轮）：写入时对可解析但疑似异常的日期给反馈（前端用 `<input type=date>`
  已规范化；可在 API 层对非空且无法解析的 expiry 返回 400，或在导入检查报告里列
  "无法识别的日期"维度），需要产品取舍（"长期有效"类自由文本要继续放行）。
- 验证：default+personal 全量绿、三 tag 编译、gofmt/vet 净。

### 2026-09-09：合并重复供应商数据丢失修复——无项目名绩效记录折叠、同资质证书号/到期日被丢

延续"审计时间驱动/破坏性核心逻辑"的方向，本轮走查**人工合并（合并重复供应商）**——
该操作把重复档案的集合数据并入主体后归档重复方，任何静默丢失都是不可接受的。集合并集
（品类/绩效/附件/资质/产品线/自定义字段/SharedWith）总体按"主体优先、加法并入、重试
幂等"实现，护栏（自合并/归档/黑名单拒绝）齐全；发现两处**重复侧数据被静默丢弃**：

- **无项目名绩效记录全部折叠成一条**：`perfKey` = `project|date`，而绩效校验只限制
  分值 0–5，**不要求项目名/日期**——前端表单允许只打分不填项目。于是主体与重复方各有
  若干"仅评分"记录时，它们的 key 全是 `"|"`，第二条起被当重复丢弃，评分也不进合并后的
  总评分（违反"合作记录归档"承诺）。修复：project/date 皆空时改用**内容指纹**
  （总分/交付/质量/配合/反馈）：不同评分或反馈的记录各自保留，字节相同的重复录入仍去重
  （保证部分失败后重试合并不翻倍）。有项目名或日期时维持 project|date 事件身份。
- **带证书号/到期日的同类型资质整条被丢**：主体记了无编号的"建筑工程施工总承包一级"，
  重复方记了同类型同等级但**带证书号与到期日**的完整记录，`qualPresent` 判定重复后直接
  跳过——证书号与到期日消失，而到期日正是 90/30/7 天临期提醒的依据。修复：谓词改为
  `qualIndex`，命中同一资质行时 `enrichQualification` **只填空白字段**（证书号/发证机关/
  发证日/到期日/verified），主体已有值永不覆盖；两张证书号都存在且不同 → 仍是两条独立
  资质（换证/物理证书不同不折叠）。
- 测试：内存层新增两例——3 条仅评分记录（4.0 / 2.0+反馈 / 交付5质量3，外加一条与主体
  字节相同）合并后保留 3 条、评分 ≈3.33；资质补全（QualsAdded=0、就地补号/到期日）+
  不同证书号仍 2 条。SQLite 层新增 `merge_persistence_test.go`：评分记录、补全资质、
  附件引用 URL（指向已归档重复方 key）跨关库重开完整保留，重复方仍归档、默认列表只见
  主体。既有 4 个合并测试不变。
- 走查确认无问题的点：附件只复制引用（下载按 key 前缀从已归档档案取 name/mime，文件
  不移动，多级合并链仍可解析）；自定义字段主体优先（共享 key 丢弃重复方值，文档明示）；
  产品线按名称并集（同名不同规格保守丢弃变体）；归档复用 Put+Delete 既有约定；并集各集合
  均按内容去重，部分失败重试不会翻倍。
- 验证：default+personal 全量绿、三 tag 编译、gofmt/vet 净。
